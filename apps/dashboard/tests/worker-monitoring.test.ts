...
describe("dashboard monitoring discovery", () => {
  it("associates a Worker with its official custom domain", () => {
    const resources = normalizeResources({
      ...
      pages: [],
      workers: [{ id: "jovlo-ai" }, { id: "journey-wave" }],
      workerDomains: [
        { hostname: "jovlo.8xd.io", service: "jovlo-ai", zone_name: "8xd.io" },
        { hostname: "journey-wave.8xd.io", service: "journey-wave", zone_name: "8xd.io" },
      ],
      d1: [],
      r2: { buckets: [] },
      kv: [],
    ...
    expect(resources.find((item) => item.id === "worker-journey-wave")).toMatchObject({
      name: "journey-wave",
      projectKey: "journey-wave",
      domain: "8xd.io",
      hostnames: ["journey-wave.8xd.io"],
    });
  });
  ...
  it("accepts only complete HTTPS health targets", () => {
    const targets = parseHealthcheckTargets(JSON.stringify([
      ...
      },
      {
        id: "journey-wave",
        name: "JourneyWave",
        url: "https://journey-wave.8xd.io/api/health",
        projectKey: "journey-wave",
        domain: "8xd.io",
      },
  ...
  it("requires an explicit healthy JSON contract instead of any 2xx response", () => {
    expect(isHealthyResponse(true, { ok: true })).toBe(true);
    expect(isHealthyResponse(true, { data: { ok: true } })).toBe(true);
